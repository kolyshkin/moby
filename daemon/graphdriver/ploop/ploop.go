...
func Init(home string, opt []string) (graphdriver.Driver, error) {
	...
	if err := ploop.Create(&cp); err != nil {
		logrus.Errorf("Can't create ploop image! Maybe some prerequisites are not met?")
		logrus.Errorf("Make sure you have ext4 filesystem in %s.", home)
		logrus.Errorf("Check that e2fsprogs and parted are installed.")
		return nil, err
	}
